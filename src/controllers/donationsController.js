...
export const donationsController = {
  ...
  async initiate(req, res, next) {
    try {
      ...
      const result = await donationsService.initiateDonation(req.validated, { host, origin, protocol });
      res.status(201).json({
        success: true,
        data: result,
  ...
  async ccavenueResponse(req, res, next) {
    try {
      ...
      const result = await donationsService.handleCCAvenueResponse(encResp, { host, origin, protocol });
      
      // Redirect browser directly to frontend success or failure page
      return res.redirect(result.redirectUrl);
    } catch (error) {
      console.error('[CCAvenue Response Callback Error]:', error);
      const isLive = req.get('host') && !req.get('host').includes('localhost');
      const frontendUrl = process.env.FRONTEND_URL || (isLive ? 'https://mumbaicharaja.co' : 'http://localhost:3000');
